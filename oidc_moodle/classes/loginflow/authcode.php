    ...
    protected function handlelogin(string $oidcuniqid, array $authparams, array $tokenparams, jwt $idtoken) {
        ...
        if (!empty($tokenrec)) {
            ...
            */
            ...
            // Custom modification: Match existing Moodle users by email
            $idpemail = $idtoken->claim('email');
            if (empty($idpemail)) {
                $userinfo = $this->get_userinfo($username);
                $idpemail = !empty($userinfo['email']) ? $userinfo['email'] : null;
            }
            if (!empty($idpemail)) {
                $matchedbyemail = $DB->get_record('user', ['email' => $idpemail, 'deleted' => 0], '*', IGNORE_MULTIPLE);
                if (!empty($matchedbyemail)) {
                    // User found by email! Use their existing Moodle username.
                    $username = $matchedbyemail->username;
                    
                    // Since they were previously a manual login, we must convert them to an OIDC login
                    // otherwise the authentication check will reject them.
                    if ($matchedbyemail->auth !== 'oidc') {
                        $DB->update_record('user', (object)['id' => $matchedbyemail->id, 'auth' => 'oidc']);
                    }
                }
            }
