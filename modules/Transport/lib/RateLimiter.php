 ...
 * Protects against DoS and brute-force attacks by limiting requests per IP/API Key.
 * 
 * Usage:
...
namespace Gibbon\Module\Transport;

class RateLimiter
{
    // Default limits
    private const DEFAULT_LIMIT = 60;          // 60 requests per minute for generic IPs
    private const API_KEY_LIMIT = 500;         // 500 requests per minute for valid API keys
     ...
     * Constructor
     * 
     * @param mixed $pdo PDO connection or null for standalone mode
     * @param string|null $apiKey Optional API key for higher limits
     */
    public function __construct($pdo = null, $apiKey = null)
    {
        $this->pdo = $pdo;
        $this->currentLimit = $apiKey ? self::API_KEY_LIMIT : self::DEFAULT_LIMIT;
        ...
        if ($this->storageType === self::STORAGE_FILE) {
            $this->fileDir = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'ratelimiter';
     ...
     * @param string|null $apiKey Optional API key for elevated limits
     * @return bool True if allowed, false if rate limited
     */
    public function allowRequest($identifier = null, $apiKey = null)
    {
        ...
        // Use API key as identifier if provided and valid
        if ($apiKey) {
            $identifier = 'apikey:' . $apiKey;
            $this->currentLimit = self::API_KEY_LIMIT;
        } else {
            $identifier = 'ip:' . $identifier;
            $this->currentLimit = self::DEFAULT_LIMIT;
        }
     ...
     */
    private function checkRedis($key, $now, $windowStart)
    {
        try {
     ...
     * @return bool True if allowed
     */
    private function checkApcu($key, $now, $windowStart)
    {
     ...
     * @return bool True if allowed
     */
    private function checkFile($identifier, $now, $windowStart)
    {
