...
class base {
    ...
    protected function process_idtoken($idtoken, $orignonce = '') {
        ...
        if (empty($sub)) {
            ...
            throw new moodle_exception('errorauthinvalididtoken', 'auth_oidc', null, null, 'Missing sub claim');
        }
        ...
        if (!empty($orignonce) && (empty($receivednonce) || $receivednonce !== $orignonce)) {
            ...
            throw new moodle_exception('errorauthinvalididtoken', 'auth_oidc', null, null, 'Nonce mismatch');
        }
    ...
    protected function createtoken(
        ...
        if (empty($oidcusername) || empty($useridentifier)) {
            $debugmsg = 'Empty username or useridentifier. oidcusername: ' . json_encode($oidcusername) . 
                        ', useridentifier: ' . json_encode($useridentifier) . 
                        ', claims: ' . print_r((array)$idtoken, true);
            throw new moodle_exception('errorauthinvalididtoken', 'auth_oidc', null, null, $debugmsg);
        }
